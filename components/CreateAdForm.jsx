...
const CreateAdForm = ({ isOpen, onClose, onAdCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      if (addNotification) {
        addNotification({ tipo: 'error', titulo: 'Errores en el formulario', mensaje: errorMessages });
      }
      return;
    }
    ...
    try {
      if (isDemoUser === true) {
        const created = {
          ...
          estado: 'activo',
          tracking: { impresiones: 0 },
        };
        if (addNotification) {
          addNotification({ tipo: 'success', titulo: 'Anuncio creado', mensaje: 'El anuncio ha sido creado exitosamente' });
        }
        onAdCreated?.(created);
        onClose();
        setFormData({
      ...
      if (addNotification) {
        addNotification({ tipo: 'error', titulo: 'Error', mensaje: error.message || 'Error al crear el anuncio' });
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            {notice && (
              <div className="mb-3 bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-yellow-800 text-sm">
                {notice}
              </div>
            )}
