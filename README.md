...
- Módulo `transacciones` mínimo operativo:
  - `GET /api/transacciones` (auth)
  - `POST /api/transacciones` (auth advertiser/admin)
  - `POST /api/transacciones/webhook`
- Landing page con tema oscuro/claro y navegación principal.
- Flujo comercial MVP operativo: `DRAFT → pago Stripe → SUBMITTED → PUBLISHED` + tracking de clics enriquecido (`GET /api/campaigns/:id/click`) y consulta de métricas (`GET /api/campaigns/:id/tracking`).

### ⚠️ Parcial / pendiente
- Módulos aún en placeholder (`501 NOT_IMPLEMENTED`): `canales`, `estadisticas`, `lists`.
- Módulos MVP nuevos: `anuncios` (CRUD mínimo), `notifications` (list/create/mark-read) y `files` (CRUD mínimo).
