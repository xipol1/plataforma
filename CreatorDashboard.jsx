...
const CreatorDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      setLoading(true);
      if (isDemo) {
        setStats(demoCreatorDashboard.stats);
        setAllAds(demoCreatorDashboard.ads);
        setPendingAds(demoCreatorDashboard.ads.filter((a) => a.estado === 'pendiente'));
        setChannels(demoCreatorDashboard.channels);
        setRecentActivities(
          demoCreatorDashboard.recentActivities.map((a) => ({
            content: a.content,
  ...
  const chartData = {
    labels: stats?.canalesPorPlataforma ? Object.keys(stats.canalesPorPlataforma) : [],
    datasets: [
      {
        label: 'Canales',
        data: stats?.canalesPorPlataforma ? Object.values(stats.canalesPorPlataforma) : [],
  ...
  const pendingAdsColumns = [
    ...
    },
    {
      ...
      accessor: 'estado',
      render: (row) => (
        <Badge variant="warning">{row.estado}</Badge>
      ),
    },
    {
      header: 'Acciones',
      render: () => (
        <Button size="sm" variant="outline" onClick={() => setActiveItem('ads')}>
          Ver detalle
        </Button>
      ),
    },
              ...
              <div className="xl:col-span-2 flex flex-col gap-6">
                <div className="bg-white rounded-xl shadow-card p-6">
                  ...
                  {pendingAds.length > 0 ? (
                    ...
                    <div className="text-center py-6 text-gray-500">No tienes solicitudes pendientes.</div>
                  )}
                </div>
                
                <div className="bg-white rounded-xl shadow-card p-6">
                  <div className="flex flex-col gap-4">
                    <h2 className="text-lg font-medium text-gray-900">Canales por Plataforma</h2>
                    <div className={stats?.canalesPorPlataforma ? "h-64 relative" : ""}>
