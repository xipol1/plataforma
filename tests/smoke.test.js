...
describe('Smoke API checks', () => {
  ...
  test('POST /api/auth/registro valida fortaleza de contraseÃ±a', async () => {
  ...
  test('POST /api/campaigns con anunciante crea campaÃ±a', async () => {
    const res = await request(app)
      .post('/api/campaigns')
      .set('Authorization', `Bearer ${advertiserToken}`)
      .send({ titulo: 'CampaÃ±a Q2', presupuesto: 500 });
  ...
  test('GET /api/campaigns con anunciante lista campaÃ±as', async () => {
    const res = await request(app)
      .get('/api/campaigns')
      .set('Authorization', `Bearer ${advertiserToken}`);
  ...
  test('POST /api/transacciones crea transacciÃ³n vÃ¡lida', async () => {
    const res = await request(app)
