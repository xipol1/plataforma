...
const validacionesRegistro = [
  ...
  body('rol')
    .isIn(['creador', 'anunciante'])
    .withMessage('Rol inválido. Debe ser creador o anunciante'),
  
  // Validaciones condicionales para perfil de creador
  body('perfilCreador.biografia')
    .if(body('rol').equals('creador'))
    .optional()
    .isLength({ max: 500 })
...
const limitarRestablecimiento = limitarIntentos({
  windowMs: 60 * 60 * 1000, // 1 hora
  max: 3, // 3 solicitudes por IP por hora
  message: {
    success: false,
