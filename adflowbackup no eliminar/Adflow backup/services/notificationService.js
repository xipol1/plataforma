...
class NotificationService extends EventEmitter {
  ...
  async enviarNotificacion(datos) {
    try {
      ...
      if (canalesFiltrados.length === 0) {
        console.log(`Usuario ${usuarioId} tiene deshabilitadas las notificaciones para ${tipo}`);
        return null;
      }
  ...
  async enviarPush(usuario, datos) {
    try {
      ...
      console.log(`📱 Push notification enviada a ${usuario.email}:`, payload.notification.title);
