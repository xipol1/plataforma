...
exports.registro = async (req, res) => {
  try {
    ...
    // Validar rol
    if (!['creador', 'anunciante', 'admin'].includes(rol)) {
      return res.status(400).json({
        success: false,
    ...
    const nuevoUsuario = new Usuario({
      ...
      perfilAnunciante: rol === 'anunciante' ? perfilAnunciante : undefined
    });
    ...
    res.status(201).json({
      ...
      data: {
        usuario: {
          ...
          verificado: nuevoUsuario.verificacion.emailVerificado
        },
        token,
        refreshToken
      }
...
exports.actualizarPerfil = async (req, res) => {
  try {
    ...
    const camposProhibidos = ['password', 'email', 'rol', 'verificacion', 'refreshTokens'];
