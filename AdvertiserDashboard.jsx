...
import ChannelMarketplace from './components/ChannelMarketplace';
...
const AdvertiserDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      setLoading(true);
      if (isDemo) {
        setStats(demoAdvertiserDashboard.stats);
        setActiveAds(demoAdvertiserDashboard.activeAds);
        setAllAds(demoAdvertiserDashboard.allAds);
        setRecentActivities(
          demoAdvertiserDashboard.recentActivities.map((a) => ({
            content: a.content,
            timestamp: a.timestamp,
            icon: getIconForNotification(a.type),
          }))
        );
        setRecommendedChannels(demoChannels.filter((c) => c.verificado).slice(0, 3));
      ...
      const [statsRes, adsRes, notificationsRes, channelsRes, allChannelsRes] = await Promise.all([
        ...
        apiService.searchChannels({ limite: 3, verificado: 'true' }),
        apiService.searchChannels({ limite: 20 })
      ]);
      ...
      if (channelsRes.success) {
        const next = channelsRes.channels || [];
        setRecommendedChannels(Array.isArray(next) && next.length ? next : demoChannels.filter((c) => c.verificado).slice(0, 3));
      } else {
        setRecommendedChannels(demoChannels.filter((c) => c.verificado).slice(0, 3));
      }
      
      if (allChannelsRes.success) {
        const next = allChannelsRes.channels || [];
        setAvailableChannels(next);
      } else {
        setAvailableChannels(demoChannels);
      }
    } catch (error) {
        ...
        <div className="flex flex-col gap-6">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <h1 className="text-2xl font-bold text-gray-900 truncate">Bienvenido, {user?.nombre || 'Anunciante'}</h1>
            ...
            <div className="xl:col-span-2 flex flex-col gap-6">
              ...
              <div className="bg-white rounded-xl shadow-card p-6">
                <div className="flex flex-col gap-4">
                  <h2 className="text-lg font-medium text-gray-900">Distribución de Anuncios por Estado</h2>
                  <div className={stats?.anunciosPorEstado ? "h-64 relative" : ""}>
                    {stats?.anunciosPorEstado ? (
                      <Doughnut data={chartData} options={chartOptions} />
                    ) : (
                      <div className="bg-gray-100 rounded-lg flex items-center justify-center py-10">
                ...
                <div className="flex flex-col gap-4">
                  {recommendedChannels.map((channel, index) => (
                    <div key={index} className="p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                      {(() => {
                        const nombre = channel?.nombre || channel?.name || 'Canal';
                        const plataforma = channel?.plataforma || channel?.platform || '';
                        const tematica = channel?.tematica || channel?.category || '';
                        const audiencia = channel?.audiencia ?? channel?.subscriberCount ?? 0;
                        const moneda = channel?.moneda || 'EUR';
                        const precio = Number(channel?.precio ?? channel?.price ?? 0);

                        return (
                          <div className="flex flex-col gap-3">
                            <div className="flex items-start justify-between gap-4">
                              <h3 className="font-medium text-gray-900">{nombre}</h3>
                              <Badge variant="info">{String(plataforma).toUpperCase()}</Badge>
                            </div>
                            <div className="flex items-center justify-between gap-4 text-sm">
                              <span className="text-gray-600">{Number(audiencia || 0).toLocaleString()} audiencia</span>
                              <span className="font-semibold text-gray-900">{moneda === 'EUR' ? `${Math.round(precio)}€ / post` : `${moneda} ${Math.round(precio)} / post`}</span>
                            </div>
                            <div className="flex items-center justify-between gap-4 text-sm">
                              <span className="text-gray-600">Temática</span>
                              <span className="font-medium text-primary-600">{tematica}</span>
                            </div>
