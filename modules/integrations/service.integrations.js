...
class IntegrationsService {
  /**
   * Obtener todas las campañas (anuncios) asignadas a un partner (Con paginación)
   * @param {string} partnerId - ID del partner autenticado
   * @param {Object} options - Opciones de paginación y filtrado (page, limit, status)
   */
  async getCampaigns(partnerId, options = {}) {
    try {
      ...
      if (options.status) {
        query.estado = options.status;
      }

      const total = await Anuncio.countDocuments(query);
      const total_pages = Math.ceil(total / limit);

      const campaigns = await Anuncio.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
  ...
  async confirmPublication(campaignId, partnerId, data) {
    try {
      ...
      // Disparar Webhook de publicación
      const partner = await Partner.findById(partnerId);
      if (partner) {
        webhookService.sendWebhook(partner, 'campaign.published', {
