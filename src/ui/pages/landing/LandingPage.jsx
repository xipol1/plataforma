...
const products = [
  {
    ...
    description: 'Compra espacios en comunidades auditadas con mÃ©tricas reales y riesgo reducido.',
    icon: 'ðŸ›¡ï¸'
  },
  {
    title: 'Pagos protegidos',
    description: 'Checkout seguro con liberaciÃ³n escalonada y trazabilidad por campaÃ±a.',
    icon: 'ðŸ’³'
  },
  {
    title: 'AnalÃ­tica en vivo',
...
const listings = [
  { name: 'Crypto Alpha Signals', platform: 'Telegram', members: '120k audiencia', rating: '4.9', price: 'â‚¬450 / post' },
  { name: 'Gaming Deals Hub', platform: 'Discord', members: '150k audiencia', rating: '4.8', price: 'â‚¬650 / post' },
  { name: 'Ecom Growth ES', platform: 'WhatsApp', members: '80k audiencia', rating: '5.0', price: 'â‚¬390 / post' }
]

const categories = ['Trading', 'Cripto', 'Gaming', 'IA', 'Ecommerce', 'EducaciÃ³n']
    ...
    <div className={isDark ? 'relative overflow-hidden bg-[#0a0a12] text-white' : 'relative overflow-hidden bg-slate-50 text-slate-900'}>
      ...
      <section className="relative mx-auto grid w-full max-w-[1240px] gap-12 px-4 py-16 md:px-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:py-24">
        <div>
          ...
          <p className={isDark ? 'mt-5 max-w-2xl text-base leading-7 text-indigo-100/80 md:text-lg' : 'mt-5 max-w-2xl text-base leading-7 text-slate-600 md:text-lg'}>
            Inspirado en UX financiera moderna: descubrimiento rÃ¡pido, pagos seguros y operaciÃ³n de campaÃ±as de extremo
              ...
              <article key={channel.name} className={isDark ? 'rounded-xl border border-white/10 bg-[#101626] p-4 transition hover:border-emerald-400/60' : 'rounded-xl border border-slate-200 bg-slate-50 p-4 transition hover:border-emerald-400'}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className={isDark ? 'text-xs text-indigo-100/70' : 'text-xs text-slate-500'}>{channel.platform} Â· {channel.members}</p>
                  </div>
                  <span className={isDark ? 'rounded-md bg-white/10 px-2 py-1 text-xs text-indigo-100' : 'rounded-md bg-slate-200 px-2 py-1 text-xs text-slate-700'}>â˜… {channel.rating}</span>
                </div>
      ...
      <section id="categories" className="mx-auto w-full max-w-[1240px] px-4 py-12 md:px-10 md:py-16">
        <div className="mb-6 flex items-end justify-between">
          <h2 className="font-['Syne'] text-3xl font-extrabold">CategorÃ­as premium</h2>
          <a href="#featured" className="text-sm text-emerald-300 hover:text-emerald-200">Ver todas â†’</a>
        </div>
      ...
      <section id="featured" className="mx-auto w-full max-w-[1240px] px-4 pb-16 md:px-10 md:pb-24">
        <div className="rounded-3xl border border-indigo-300/20 bg-gradient-to-r from-[#1a1f3f] via-[#14243d] to-[#143829] p-8 md:p-12">
          ...
          <h2 className="mt-3 font-['Syne'] text-3xl font-extrabold md:text-4xl">Tu operaciÃ³n de campaÃ±as como una fintech de marketing</h2>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-indigo-100/80 md:text-base">
            Workflows aprobatorios, gestiÃ³n multi-equipo, presupuesto por unidad de negocio y soporte prioritario con
