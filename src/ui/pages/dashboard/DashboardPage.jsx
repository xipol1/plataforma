    ...
    <div className="grid gap-4">
      ...
      {!loading && !error && (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <Card label="Campañas" value={resumen.campaigns} />
          <Card label="Presupuesto" value={`€ ${resumen.presupuesto.toFixed(2)}`} />
          <Card label="Transacciones" value={resumen.transacciones} />
          <Card label="Monto movido" value={`€ ${resumen.montoMovido.toFixed(2)}`} />
        </div>
      )}

      {!loading && !error && campaigns.length === 0 && transactions.length === 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-4 text-sm text-gray-600">
          Aún no tienes campañas ni transacciones. Crea tu primera campaña para comenzar.
        </div>
      )}
